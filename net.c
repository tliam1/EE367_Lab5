...
if (link_num < 1) { 
	...
	for (i=0; i<link_num; i++) {
		...
		if (link_type == 'P') {
         ...
         fscanf(fp," %d %s %d %s %d ", &node0, &(g_net_link[i].domain0), &port0, &(g_net_link[i].domain1), &port1);
         g_net_link[i].type = SOCKET;
         g_net_link[i].socket_node = node0;
...
for (i=0; i<g_net_link_num; i++) {
	if (g_net_link[i].type == PIPE) {
       ...
       printf("   Link (%d, %s %d) SOCKET\n",
            g_net_link[i].socket_node,
            g_net_link[i].domain0,
            g_net_link[i].socket_port0);
	}
}
